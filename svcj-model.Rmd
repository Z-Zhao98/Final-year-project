---
title: "SVCJ model estimation"
output: html_notebook
---
Adapted from Perez (2018) and Hout et al. (2020)
By Zhao Zhun 
...
# training.df <- bitP.df[1:1462,]
#test.df <- bitP.df[1463:1583,]
# 
# train_bitP.ts <- ts(data = as.numeric(training.df$Adj.Close),frequency = 365, start = c(2017,as.numeric(format(train_dates[1], "%j"))))
#test_bitP.ts <- ts(data = as.numeric(test.df$Adj.Close),frequency = 365, start = c(2021,as.numeric(format(test_dates[1], "%j"))))
...
logP <- log(training.df$Adj.Close)
...
BTC
```{r warning=FALSE}
BTC_result <- svcj_model(btcP.df)
```

```{r warning=FALSE}
BTC_result2 <- svcj_model(btcP.df)
```

```{r}
BTC_result$parameters
#save(result,file="btc_svcj.Rda")
```
...
```
```{r}
save(BTC_result,file="btc_svcj.Rda")
```

ETH nad lTC
```{r warning=FALSE}
ETH_result <- svcj_model(ethP.df)
save(ETH_result,file="eth_svcj.Rda")

LTC_result <- svcj_model(ltcP.df)
save(LTC_result,file="LTC_svcj.Rda")
```



```{r}
ETH_result$parameters
LTC_result$parameters
```
...
-----analysis--------
```{r}
btcP.df$Date<-as.Date(btcP.df$Date)
btc_train.df <-btcP.df[1:1462,]

ethP.df$Date<-as.Date(ethP.df$Date)
eth_train.df <-ethP.df[1:1462,]

ltcP.df$Date<-as.Date(ltcP.df$Date)
ltc_train.df <-ltcP.df[1:1462,]
```
...
plot
```{r}
plot(BTC_result$param_evolution$mu, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(BTC_result$param_evolution$lambda, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(BTC_result$param_evolution$rho, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(BTC_result$param_evolution$sigma_y, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(BTC_result$param_evolution$sigma_v, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(BTC_result$volatility ~ btc_train.df$Date[-1], type = "l", xaxt = "n", col = "blue", lwd = 1, cex.axis = .8, xlab = "Date", ylab = "Volatility")
  axis.Date(1, at=seq(min(btc_train.df$Date[-1]), max(btc_train.df$Date[-1]), by="2 mon"), format="%m-%y", cex.axis = .8)
  
  l1 <- length(BTC_result$jumps_price)
  l2 <- length(btc_train.df$Date)
  plot(BTC_result$jumps_price ~ btc_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(btc_train.df$Date[(l2-l1+1):l2]), max(btc_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
  plot(BTC_result$jumps_volatility ~ btc_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(btc_train.df$Date[(l2-l1+1):l2]), max(btc_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
...
```{r}
print(paste0("MSE = ", round(mean((BTC_result$residuals)^2),3)))
```
ETH plot
```{r}
plot(ETH_result$param_evolution$mu, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration",ylab="mu")
plot(ETH_result$param_evolution$lambda, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(ETH_result$param_evolution$rho, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(ETH_result$volatility ~ eth_train.df$Date[-1], type = "l", xaxt = "n", col = "blue", lwd = 1, cex.axis = .8, xlab = "Date", ylab = "Volatility")
  axis.Date(1, at=seq(min(eth_train.df$Date[-1]), max(eth_train.df$Date[-1]), by="2 mon"), format="%m-%y", cex.axis = .8)
  
  l1 <- length(ETH_result$jumps_price)
  l2 <- length(eth_train.df$Date)
  plot(ETH_result$jumps_price ~ eth_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(eth_train.df$Date[(l2-l1+1):l2]), max(eth_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
  plot(ETH_result$jumps_volatility ~ eth_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(eth_train.df$Date[(l2-l1+1):l2]), max(eth_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
```

```{r}
print(paste0("MSE = ", round(mean((ETH_result$residuals)^2),3)))
```
...
```{r}
plot(LTC_result$param_evolution$mu, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(LTC_result$param_evolution$lambda, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(LTC_result$param_evolution$rho, type = "l", col = "blue", lwd = 1, cex.axis = .8, xlab = "Iteration")
plot(LTC_result$volatility ~ ltc_train.df$Date[-1], type = "l", xaxt = "n", col = "blue", lwd = 1, cex.axis = .8, xlab = "Date", ylab = "Volatility")
  axis.Date(1, at=seq(min(ltc_train.df$Date[-1]), max(ltc_train.df$Date[-1]), by="2 mon"), format="%m-%y", cex.axis = .8)
  
  l1 <- length(LTC_result$jumps_price)
  l2 <- length(ltc_train.df$Date)
  plot(LTC_result$jumps_price ~ ltc_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(ltc_train.df$Date[(l2-l1+1):l2]), max(ltc_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
  plot(LTC_result$jumps_volatility ~ ltc_train.df$Date[(l2-l1+1):l2], type = "l", xaxt = "n", col = "blue", xlab = "Date", ylab = "")
  axis.Date(1, at=seq(min(ltc_train.df$Date[(l2-l1+1):l2]), max(ltc_train.df$Date[(l2-l1+1):l2]), by="2 mon"), format="%m-%y", cex.axis = .9)
```
```{r}
print(paste0("MSE = ", round(mean((LTC_result$residuals)^2),3)))
```
...
```{r}
#result_set <- c(BTC_result,ETH_result,LTC_result)
#for (i in 1:3){
qqnorm(y = (BTC_result$residuals), xlim = c(-4, 4), ylim = c(-4, 4), col = "blue", cex = 0.8,ylab="", xlab = "")
  abline(a = 0, b = 1, col = "red", lwd = 2)
  title(ylab = "Quantiles of Input Sample", line=2)
  title(xlab = "Standard Normal Quantiles", line=2)
  
qqnorm(y = (ETH_result$residuals), xlim = c(-4, 4), ylim = c(-4, 4), col = "blue", cex = 0.8,ylab="", xlab = "")
  abline(a = 0, b = 1, col = "red", lwd = 2)
  title(ylab = "Quantiles of Input Sample", line=2)
  title(xlab = "Standard Normal Quantiles", line=2)
  
qqnorm(y = (LTC_result$residuals), xlim = c(-4, 4), ylim = c(-4, 4), col = "blue", cex = 0.8,ylab="", xlab = "")
  abline(a = 0, b = 1, col = "red", lwd = 2)
  title(ylab = "Quantiles of Input Sample", line=2)
  title(xlab = "Standard Normal Quantiles", line=2)
#}
```
```{r}
shapiroTest(BTC_result$residuals)
shapiroTest(ETH_result$residuals)
shapiroTest(LTC_result$residuals)
```
```{r}
```
...
```{r}
train_startDate = as.Date("2017-11-09")
train_endDate = as.Date("2021-05-31")

training.df <- bitP.df[1:1300,]
# training.ts <- ts(data = as.numeric(training.df$Adj.Close),frequency = 365, start = c(2017,as.numeric(format(train_dates[1], "%j"))))
Y <- diff(training.df$Adj.Close)/training.df$Adj.Close[-length(training.df$Adj.Close)]*sqrt(250)
#  logP <- log(training.df$Adj.Close)
#  Y <- diff(logP)*sqrt(250)
```
